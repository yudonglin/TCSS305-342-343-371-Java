...
public class HumanTest {
                ...
                if (destinationTerrain == Terrain.GRASS) {
                    ...
                    if (currentLightCondition == Light.GREEN) {
                        assertFalse(human.canPass(destinationTerrain, currentLightCondition),
                                "Human should NOT be able to pass " + destinationTerrain + ", with light " + currentLightCondition);
                    } else { // light is yellow or red
                        assertTrue(human.canPass(destinationTerrain, currentLightCondition),
                                "Human should be able to pass " + destinationTerrain + ", with light " + currentLightCondition);
                    }
    ...
    public void testChooseDirectionSurroundedByGrass() {
        ...
        assertTrue(seenWest && seenNorth && seenEast,
                "Human chooseDirection() fails to select randomly " + "among all possible valid choices!");
        ...
        for (final Terrain t : Terrain.values()) {
            if (t != Terrain.GRASS && t != Terrain.CROSSWALK) {
                ...
                assertEquals(Direction.SOUTH, human.chooseDirection(neighbors),
                        "Human chooseDirection() failed " + "when reverse was the only valid choice!");
            }
        }
    }
        ...
        for (final Direction d : Direction.values()) {
            ...
            if (d == Direction.EAST) {
                assertNotEquals(Direction.WEST, human.chooseDirection(neighbors),
                        "A human near a crosswalk and facing " + d + " should not reverse direction!");
            } else {
                assertEquals(Direction.WEST, human.chooseDirection(neighbors), "A human near a crosswalk and facing " + d + " chose a wrong direction!");
            }
        }
    }
}
